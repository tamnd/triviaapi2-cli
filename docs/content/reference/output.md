...
describes what users get. Pick a format with `-o`, or let triviaapi2 choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
triviaapi2 <command> -o table   # aligned columns for reading
triviaapi2 <command> -o jsonl   # one JSON object per line, for piping
triviaapi2 <command> -o json    # a single JSON array
triviaapi2 <command> -o csv     # spreadsheet friendly
triviaapi2 <command> -o tsv     # tab-separated
triviaapi2 <command> -o url     # just the URL column
triviaapi2 <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
triviaapi2 <command> --fields id,title,url
```
...
```bash
triviaapi2 <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
triviaapi2 <command>            # a table, because this is a terminal
triviaapi2 <command> | wc -l    # JSONL, because this is a pipe
```
